define(["require", "exports", "./Matrix", "./Control"], function (require, exports, Matrix_1, Control_1) {
    "use strict";
    ...
    var TransformTool = /** @class */ (function () {
        function TransformTool(container) {
            ...
            this.startMatrix = new Matrix_1.Matrix();
            this.regStartU = 0.5;
            this.regStartV = 0.5;
            this.startX = 0;
            this.startY = 0;
            // trnasforms to apply
            this.preMatrix = new Matrix_1.Matrix();
            this.postMatrix = new Matrix_1.Matrix();
            // where interaction ends
            this.endMatrix = new Matrix_1.Matrix();
            this.regEndU = 0.5;
            this.regEndV = 0.5;
            this.endX = 0;
            this.endY = 0;
        ...
        TransformTool.prototype.applyControl = function () {
            if (this.control) {
                ...
                switch (this.control.type) {
                    case Control_1.ControlType.SCALE: {
                        x = (cu + this.dU) / cu;
                        y = (cv + this.dV) / cv;
                        this.preMatrix.scale(x, y);
                        break;
                    }
                    case Control_1.ControlType.SCALE_X: {
                        x = (cu + this.dU) / cu;
                        this.preMatrix.scale(x, 1);
                        break;
                    }
                    case Control_1.ControlType.SCALE_Y: {
                        y = (cv + this.dV) / cv;
                        this.preMatrix.scale(1, y);
                        break;
                    }
                    case Control_1.ControlType.SCALE_UNIFORM: {
                        x = (cu + this.dU) / cu;
                        y = (cv + this.dV) / cv;
                    ...
                    case Control_1.ControlType.SKEW_X: {
                        this.preMatrix.c = (w / h) * (this.dU / cv);
                        break;
                    }
                    case Control_1.ControlType.SKEW_Y: {
                        this.preMatrix.b = (h / w) * (this.dV / cu);
                        break;
                    }
                    case Control_1.ControlType.ROTATE_SCALE: {
                    ...
                    case Control_1.ControlType.ROTATE: {
                        // rotation in global space
